...
// Filter Functions
float32 Digital_Filter_2nd_order(float32 x_k);
// float32 Deriv_Median_Filter(float32);
...
//#define Current_Conv_Constant 1.72409e-3	// Takes into account DivBy4 and ADC_CONVERSION
//#define Current_Conv_Constant 3.44818e-3	// Takes into account DivBy2 and ADC_CONVERSION

//#define Current_Conv_Constant 3.6991e-3 // Takes into account DivBy2 and ADC_CONVERSION // New as of 2/22

//#define Current_Conv_Constant 3.4679e-3 // Takes into account DivBy2 and ADC_CONVERSION // New as of 2/22 // Take TWO

#define Current_Conv_Constant 3.7214e-3 // Takes into account DivBy2 and ADC_CONVERSION // New as of 3/3


//#define Current_Conv_Offset -14.11137

//#define Current_Conv_Offset -15.1515	// New as of 2/22

//#define Current_Conv_Offset -14.2045	// New as of 2/22 // Take TWO

#define Current_Conv_Offset -15.01433	// New as of 2/22

// Position measurement scaling and/or shifting conversion

// --------- Full Range Distance

//#define Position_Conv_Constant 1.9781e-7	// Takes into account DivBy4 and ADC_CONVERSION

//#define Position_Conv_Constant 1.9771e-7 	// New as of 2/22

#define Position_Conv_Constant 1.977094e-7 	// New as of 3/3

// #define Position_Conv_Offset 3.8677e-6

#define Position_Conv_Offset 4.411644e-6 // New as of 3/3
...
#define KP_pos 20413
#define KI_pos 168770
#define KD_pos 93.6


#define mTfplusPt5Ts  -2.287e-04
#define	plusTfplusPt5Ts 3.287e-04
#define one_div_plusTfplusPt5Ts 3.0423e+03
...
Uint16 volatile Position_Sampled_Flag;
Uint16 volatile Position_PID_cntrl_flag;
Uint16 volatile Position_samp1, Position_samp2, Position_samp3, Position_samp4;
...
main()
{
	float32 Current_Cmd[coil_num] = {1.5, 1.5};
	float32 delta_I_x;
...
// Initialize Global Variables
   ...
   IdleCount = 0;
   //Position_Sampled_Flag = 1;
   Position_Sampled_Flag = 0;
   InterruptCount = 0;
...
//--------- Configure EPWM1 ---------
   ...
   // Disable remaining H-bridges on the board
   GpioDataRegs.GPBCLEAR.bit.GPIO33 = 1;  // Enable H-bridge operation ...connect to PWMH pin
   GpioDataRegs.GPBCLEAR.bit.GPIO49 = 1;  // Enable H-bridge operation ...connect to PWMH pin
   GpioDataRegs.GPBCLEAR.bit.GPIO62 = 1;  // Enable H-bridge operation ...connect to PWMH pin
...
// Wait for in idle until new ADC samples received, then perform current and position control
   for(;;)
   {
      ...
      if(BB_cntrl_flag == 1)
      {
    	  ...
    	  if(Position_PID_cntrl_flag == 1)			// Perform Position control algorithm when flag is set
    	  {
			...
			*/
    		  ...
    		  Position_PID_cntrl_flag = 0;		// Reset flag
...
float32 Position_PID_Cntrl_x(void)
{
	...
	Error_pos_filtered = Digital_Filter_2nd_order(Error_pos);

	//Error_pos_filtered = Error_pos;   // Removes filter

	// Proportional Term
	Prop_pos = Error_pos_filtered;
	...
	{
		delta_I_x_calc = (Prop_pos*KP_pos) + Der_pos;		// PD control
		Int_pos = 0;
	}

	else
	{
		delta_I_x_calc = (Prop_pos*KP_pos) + Int_pos + Der_pos;	//PID control
	}
...
float32 Digital_Filter_2nd_order(float32 x_k) // SECOND ORDER
{
	// Input array
...
// ------------
	...
	if(Position_Sampled_Flag == 1)
		{
			Position_PID_cntrl_flag = 1;	// Set flag to perform position controller algorithm in main()
			Position_Sampled_Flag = 0;		// Clear flag

			Position_samp1 = AdcRegs.ADCRESULT6 >>4;
			Position_samp2 = AdcRegs.ADCRESULT7 >>4;
			Position_samp3 = AdcRegs.ADCRESULT8 >>4;
			Position_samp4 = AdcRegs.ADCRESULT9 >>4;
	...
	if(InterruptCount == (Pos_Sample_div))
		{
			// Next ADC cycle will convert read both position and current sensor data
			ADC_A_POS_CUR_SAMPLING;			// Macro
			Position_Sampled_Flag = 1;		// Set flag so next ADC cycle the converted position data is read
...
interrupt void  EPWM1_isr(void)
{
	...
	//GpioDataRegs.GPATOGGLE.bit.GPIO18 = 1;	// Instrumentation for timing
...
void GPIO_setup(void)
{
	   ...
	   //	GpioCtrlRegs.GPAMUX1.all = 0x00000000;	// Set all Port A to GPIO
	   //	GpioCtrlRegs.GPAMUX2.all = 0x00000000;
	   //	GpioCtrlRegs.GPADIR.all = 0xFFFFFFFF;	// Set all Port A to outputs
	   //	GpioDataRegs.GPACLEAR.all = 0xFFFFFFFF;	// Clear all bits in Port A
